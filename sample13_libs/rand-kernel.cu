...
void use_host_api(int N)
{
    ...
    CUDA_CHECK(cudaMalloc((void **)&dRand, sizeof(float) * N));
    CUDA_CHECK(cudaMalloc((void **)&dOut, sizeof(float) * N));
    ...
    // Retrieve outputs
    CUDA_CHECK(cudaMemcpy(hOut, dOut, sizeof(float) * N, cudaMemcpyDeviceToHost));
