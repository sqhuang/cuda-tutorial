...
float cuda_device_rand()
{
    ...
    if (dRand == NULL)
    {
         ...
         * state objects.
         */
        CUDA_CHECK(cudaMalloc((void **)&dRand, sizeof(float) * dRand_length));
        CUDA_CHECK(cudaMalloc((void **)&states, sizeof(curandState) *
    ...
    if (dRand_used == dRand_length)
    {
        ...
        refill_randoms<<<blocks_per_grid, threads_per_block>>>(dRand,
                dRand_length, states);
        CUDA_CHECK(cudaMemcpy(hRand, dRand, sizeof(float) * dRand_length,
