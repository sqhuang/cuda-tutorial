...
int main(int argc, char **argv)
{
    ...
    CUDA_CHECK(cudaMalloc((void **)&dComplexSamples,
            sizeof(cufftComplex) * N));

    // Transfer inputs into device memory
    CUDA_CHECK(cudaMemcpy(dComplexSamples, complexSamples,
    ...
    CUDA_CHECK(cudaMemcpy(complexFreq, dComplexSamples,
    ...
    CUDA_CHECK(cudaFree(dComplexSamples));
