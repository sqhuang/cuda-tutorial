                           ...
                           float *sample, int sampleLength)
{
    ...
    double toDeviceStart = cpuTimerInS();
    CUDA_CHECK(cudaMemcpy(d_floatInputs, h_floatInputs, sizeof(float) * N,
                     cudaMemcpyHostToDevice));
    *toDeviceTime = cpuTimerInS() - toDeviceStart;

    double kernelStart = cpuTimerInS();
    lots_of_float_compute<<<blocksPerGrid, threadsPerBlock>>>(d_floatInputs,
            N, niters, d_floatOutputs);
    CUDA_CHECK(cudaDeviceSynchronize());
    *kernelTime = cpuTimerInS() - kernelStart;

    double fromDeviceStart = cpuTimerInS();
    CUDA_CHECK(cudaMemcpy(h_floatOutputs, d_floatOutputs, sizeof(float) * N,
                     cudaMemcpyDeviceToHost));
    *fromDeviceTime = cpuTimerInS() - fromDeviceStart;
                            ...
                            double *sample, int sampleLength)
{
    ...
    double toDeviceStart = cpuTimerInS();
    CUDA_CHECK(cudaMemcpy(d_doubleInputs, h_doubleInputs, sizeof(double) * N,
                     cudaMemcpyHostToDevice));
    *toDeviceTime = cpuTimerInS() - toDeviceStart;
    ...
    *kernelTime = cpuTimerInS() - kernelStart;
    ...
    double fromDeviceStart = cpuTimerInS();
    CUDA_CHECK(cudaMemcpy(h_doubleOutputs, d_doubleOutputs, sizeof(double) * N,
                     cudaMemcpyDeviceToHost));
    *fromDeviceTime = cpuTimerInS() - fromDeviceStart;
