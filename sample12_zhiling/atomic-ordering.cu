                               ...
                               const char *label)
{
    ...
    CUDA_CHECK(cudaMemcpy(h_arr, d_arr, nToPrint * sizeof(int),
...
int main(int argc, char **argv)
{
    ...
    for (r = 0; r < runs; r++)
    {
        double start_atomic = cpuTimerInS();
        CUDA_CHECK(cudaMemset(d_shared_var, 0x00, sizeof(int)));
        atomics<<<N / block, block>>>(d_shared_var, d_values_read_atomic, N,
                                          iters);
        CUDA_CHECK(cudaDeviceSynchronize());
        atomic_mean_time += cpuTimerInS() - start_atomic;
        CUDA_CHECK(cudaMemcpy(&h_shared_var_atomic, d_shared_var, sizeof(int),
                         cudaMemcpyDeviceToHost));

        double start_unsafe = cpuTimerInS();
        CUDA_CHECK(cudaMemset(d_shared_var, 0x00, sizeof(int)));
        unsafe<<<N / block, block>>>(d_shared_var, d_values_read_unsafe, N,
                                         iters);
        CUDA_CHECK(cudaDeviceSynchronize());
        unsafe_mean_time += cpuTimerInS() - start_unsafe;
        CUDA_CHECK(cudaMemcpy(&h_shared_var_unsafe, d_shared_var, sizeof(int),
                         cudaMemcpyDeviceToHost));
    }
