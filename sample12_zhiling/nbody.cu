...
int main(int argc, char **argv)
{
    ...
    CUDA_CHECK(cudaMalloc((void **)&d_px, N * sizeof(real)));
    CUDA_CHECK(cudaMalloc((void **)&d_py, N * sizeof(real)));

    CUDA_CHECK(cudaMalloc((void **)&d_vx, N * sizeof(real)));
    CUDA_CHECK(cudaMalloc((void **)&d_vy, N * sizeof(real)));

    CUDA_CHECK(cudaMalloc((void **)&d_ax, N * sizeof(real)));
    CUDA_CHECK(cudaMalloc((void **)&d_ay, N * sizeof(real)));

    CUDA_CHECK(cudaMalloc((void **)&d_exceeded_speed, sizeof(int)));
    CUDA_CHECK(cudaMalloc((void **)&d_beyond_bounds, sizeof(int)));

    CUDA_CHECK(cudaMemcpy(d_px, h_px, N * sizeof(real), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_py, h_py, N * sizeof(real), cudaMemcpyHostToDevice));

    CUDA_CHECK(cudaMemset(d_vx, 0x00, N * sizeof(real)));
    CUDA_CHECK(cudaMemset(d_vy, 0x00, N * sizeof(real)));
#ifdef VALIDATE
    memset(host_vx, 0x00, N * sizeof(real));
    memset(host_vy, 0x00, N * sizeof(real));
#endif // VALIDATE

    CUDA_CHECK(cudaMemset(d_ax, 0x00, N * sizeof(real)));
    CUDA_CHECK(cudaMemset(d_ay, 0x00, N * sizeof(real)));
#ifdef VALIDATE
    memset(host_ax, 0x00, N * sizeof(real));
    memset(host_ay, 0x00, N * sizeof(real));
#endif // VALIDATE

    double start = cpuTimerInS();

    for (iter = 0; iter < niters; iter++)
    {
        CUDA_CHECK(cudaMemset(d_exceeded_speed, 0x00, sizeof(int)));
        CUDA_CHECK(cudaMemset(d_beyond_bounds, 0x00, sizeof(int)));
    ...
    double exec_time = cpuTimerInS() - start;
    ...
    CUDA_CHECK(cudaMemcpy(&exceeded_speed, d_exceeded_speed, sizeof(int),
                     cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(&beyond_bounds, d_beyond_bounds, sizeof(int),
                     cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_px, d_px, N * sizeof(real), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_py, d_py, N * sizeof(real), cudaMemcpyDeviceToHost));
