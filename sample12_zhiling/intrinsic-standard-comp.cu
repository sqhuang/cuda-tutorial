...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < runs; i++)
    {
        double start_standard = cpuTimerInS();
        standard_kernel<<<1, 32>>>(input_value, d_standard_out, iters);
        CUDA_CHECK(cudaDeviceSynchronize());
        mean_standard_time += cpuTimerInS() - start_standard;

        double start_intrinsic = cpuTimerInS();
        intrinsic_kernel<<<1, 32>>>(input_value, d_intrinsic_out, iters);
        CUDA_CHECK(cudaDeviceSynchronize());
        mean_intrinsic_time += cpuTimerInS() - start_intrinsic;
    }

    CUDA_CHECK(cudaMemcpy(&h_standard_out, d_standard_out, sizeof(float),
                     cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(&h_intrinsic_out, d_intrinsic_out, sizeof(float),
                     cudaMemcpyDeviceToHost));
